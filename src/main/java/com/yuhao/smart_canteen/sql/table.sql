CREATE TABLE user (
    ...
    gender VARCHAR(2) NOT NULL COMMENT '用户的性别',
    point VARCHAR(10) NOT NULL COMMENT '用户的积分',
    phone VARCHAR(20) COMMENT '用户的联系电话',
    email VARCHAR(100) UNIQUE COMMENT '用户的电子邮件地址',
    avatar VARCHAR(255) COMMENT '用户头像的存储',
    role VARCHAR(2) NOT NULL default '1'  COMMENT '用户在系统中的角色',
    reg_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '用户注册系统的日期',
    last_login TIMESTAMP COMMENT '用户最后登录系统的时间',
